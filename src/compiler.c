...
#ifdef DEBUG_PRINT_CODE
  if(!parser.had_error){
    disassembler_chunk(current_chunk(), "code");
  }
#endif
}
...
static void parse_number(int can_assign){
  (void)can_assign;
...
static void parse_string(int can_assign){
  (void)can_assign;
...
static void named_variable(Token name, int can_assign){
...
ParseRule rules[] = {
  ...
  [TOKEN_COMA]          = {NULL,         NULL,   PREC_NONE},
